...
#get all the outdated gems that need bumping
GEM_NAME_REGEX = /name: (.+)/i.freeze
all_outdated_gem_names = []
outdated_file_name = 'outdated_gems.txt'

File.open(outdated_file_name).each do |line|
  if match = line.match(GEM_NAME_REGEX)
    all_outdated_gem_names << match[1]
  end
end
outdated_gem_names = all_outdated_gem_names.uniq.map(&:downcase)
...
while true
  ...
  next_page_gh_pull_requests.each do |pr|
    # only take the dependabot PRs
    if pr.head.label.include?("zooniverse:dependabot")
...
outdated_gem_prs.each do |outdated_gem_pr|
  ...
  if !up_to_date_with_master || FORCE_PR_UPDATE
